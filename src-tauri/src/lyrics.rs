...
/// `MAPLE_LYRICS_ONLY=<boidu|netease|qq|kugou>` pins the chain to that one provider and bypasses
...
fn forced_provider() -> Option<String> {
    std::env::var("MAPLE_LYRICS_ONLY").ok().filter(|s| !s.is_empty())
}
...
async fn fetch(state: &AppState, mut req: LyricsRequest) -> (Option<Lyrics>, bool) {
    ...
    if let Some(only) = forced_provider() {
        let hit = match only.as_str() {
            ...
            other => {
                tracing::warn!(provider = other, "MAPLE_LYRICS_ONLY: unknown provider");
                Ok(None)
            }
        };
        match &hit {
...
const LRCLIB_UA: &str =
    concat!("Maple v", env!("CARGO_PKG_VERSION"), " (https://github.com/dev-Ninjaa/maple.music)");
...
mod tests {
    ...
    ///   cargo test -p maple-app --lib -- --ignored --nocapture
    ///
