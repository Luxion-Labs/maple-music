...
/// (`VL…` / `MPRE…` / `RD…`).
pub const ON_REPEAT_ID: &str = "MAPLE_ON_REPEAT";
...
fn identity_selection_key(data_sync_id: &str) -> String {
    ...
    "maple-account-identity-v1".hash(&mut hasher);
...
impl AppState {
    ...
    /// rustypipe-solo acceptance test; `MAPLE_DISABLED_CLIENTS` env overrides for quick testing.
    fn disabled_clients(&self) -> HashSet<String> {
        let raw = std::env::var("MAPLE_DISABLED_CLIENTS")
            .ok()
