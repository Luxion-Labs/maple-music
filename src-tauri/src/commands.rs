...
#[tauri::command]
#[cfg(mobile)]
pub async fn open_mini(_app: tauri::AppHandle) -> Result<(), String> {
    Err("the mini player is unavailable on mobile".to_string())
}
...
#[tauri::command]
#[cfg(mobile)]
pub async fn close_mini(_app: tauri::AppHandle) -> Result<(), String> {
    Ok(())
}
...
/// `MAPLE_ON_REPEAT` is the local auto-playlist rather than anything YouTube knows about.
///
...
pub async fn release_notes() -> Result<Vec<ReleaseNote>, String> {
    ...
    let releases: Vec<GhRelease> = crate::http::client()
        .get("https://api.github.com/repos/dev-Ninjaa/maple.music/releases?per_page=20")
        .header("User-Agent", concat!("Maple/", env!("CARGO_PKG_VERSION")))
