...
mod imp {
    ...
    pub fn set_playing(_app: &AppHandle, _playing: bool) {}
}
...
mod imp {
    ...
    static HANDLE: OnceLock<Handle<MapleTray>> = OnceLock::new();
    ...
    pub fn init(app: &AppHandle) -> tauri::Result<()> {
        let tray = MapleTray { app: app.clone(), playing: false, icon: icon_pixmap(app) };
...
mod imp {
    ...
    pub fn init(app: &AppHandle) -> tauri::Result<()> {
        let show = MenuItem::with_id(app, "show", "Show Maple", true, None::<&str>)?;
