//! Maple Tauri app. Wires transport + player + db + orchestrator behind the command boundary.
...
pub fn run() {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // MAPLE_FORCE_GPU=1 to skip the workaround and check whether the AppImage still
        // white-screens. If it renders, delete this block.
        if std::env::var_os("APPIMAGE").is_some()
            && std::path::Path::new("/dev/nvidiactl").exists()
            && std::env::var_os("WEBKIT_DISABLE_DMABUF_RENDERER").is_none()
            && std::env::var_os("MAPLE_FORCE_GPU").is_none()
        {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,maple_app=debug".into()),
        )
        .init();
    ...
    builder
        ...
        .setup(|app| {
            ...
            let db = Arc::new(Db::open(&data_dir.join("maple.sqlite")).expect("open sqlite"));
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::playlists_continuation,
            commands::playlists_delete,
            commands::playlists_reorder,
            commands::lyrics,
            commands::sign_in_cookie,
            commands::sign_in_google,
            commands::google_suggest_account,
            commands::sign_out,
            commands::account_menu,
            commands::channel_switcher,
            commands::switch_channel,
            commands::listen_together_create,
            commands::listen_together_join,
            commands::listen_together_leave,
            commands::listen_together_state,
            commands::list_local_folders,
            commands::add_local_folder,
            commands::remove_local_folder,
            commands::scan_local_folders,
            commands::tray_show,
            commands::proxy_set,
            ...
            // Discord integration (Android Gateway, desktop uses discord.rs local IPC)
            commands::discord_connect,
            commands::discord_update_activity,
            commands::discord_disconnect,
        ])
