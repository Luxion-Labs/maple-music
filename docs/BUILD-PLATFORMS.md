# Building Maple on each platform

Maple is a Tauri 2 app (Rust core + SvelteKit SPA) that dynamically links **libmpv** (mpv API
...
cargo tauri build            # → target/release/bundle/rpm/maple-*.rpm
```
...
6. **Build:**
   ```powershell
   cd ui; pnpm build; cd ..
   cargo tauri build          # → target/release/bundle/{msi,nsis}/maple_*.{msi,exe}
   ```
...
4. **Build:**
   ```bash
   cd ui && pnpm build && cd ..
   cargo tauri build          # → target/release/bundle/{macos,dmg}/maple.{app,dmg}
   ```
5. **Bundle the dylibs, all of them, then re-sign.** What comes out of step 4 runs on *your* machine
   ...
   APP=target/release/bundle/macos/maple.app
   dylibbundler -cd -of -b -x "$APP/Contents/MacOS/maple" \
