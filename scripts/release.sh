...
#
# Requires: the private signing key at ~/.tauri/maple.key, `gh` authed, jq, curl.
set -euo pipefail
cd "$(dirname "$0")/.."

REPO="dev-Ninjaa/maple.music"
KEY="${TAURI_SIGNING_PRIVATE_KEY_FILE:-$HOME/.tauri/maple.key}"
...
if ! cargo tauri build --bundles rpm; then
  ...
  echo "         gh release upload $TAG target/release/bundle/rpm/maple-$VERSION-*.rpm --clobber --repo $REPO" >&2
  exit 1
fi
...
RPM="$(ls target/release/bundle/rpm/maple-${VERSION}-*.rpm 2>/dev/null | head -1)"
