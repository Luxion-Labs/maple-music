#!/usr/bin/env bash
# Check a Maple AppDir on a distro that isn't the build host. Runs INSIDE a container, with the
# AppDir mounted at /app, and installs the desktop packages it needs itself.
#
#   podman run --rm -v "$PWD/target/release/bundle/appimage/maple.AppDir:/app:ro,z" \
