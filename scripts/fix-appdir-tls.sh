...
APPDIR="$(readlink -f "$BUNDLE/maple.AppDir" 2>/dev/null || true)"
[ -n "$APPDIR" ] && [ -d "$APPDIR" ] || {
  echo "no AppDir at $BUNDLE/maple.AppDir — run \`cargo tauri build --bundles appimage\` first"; exit 1; }
APPIMAGE="$(ls "$BUNDLE"/maple_*.AppImage 2>/dev/null | head -1 || true)"
[ -n "$APPIMAGE" ] || { echo "no maple_*.AppImage in $BUNDLE"; exit 1; }
...
# Maple: the value written above is unusable — it contains a literal newline and an absolute path
